Imports System.IO

Public Class Magicbytes

    ' Magic bytes dictionary
    Private ReadOnly magicBytes As New Dictionary(Of String, String) From {
        {"23 21", "sh"},
        {"02 00 5A 57 52 54", "cwk"},
        {"00 00 02 00 06 04 06 00", "wk1"},
        {"00 00 1A 00 00 10 04 00", "wk3"},
        {"00 00 1A 00 02 10 04 00", "wk4"},
        {"00 00 1A 00 05 10 04", "123"},
        {"00 00 03 F3", "amiga"},
        {"00 00 49 49 58 50 52", "qxd"},
        {"50 57 53 33", "psafe3"},
        {"D4 C3 B2 A1", "pcap"},
        {"4D 3C B2 A1", "pcap"},
        {"0A 0D 0D 0A", "pcapng"},
        {"ED AB EE DB", "rpm"},
        {"53 51 4C 69 74 65 20 66", "sqlite"},
        {"53 50 30 31", "bin"},
        {"49 57 41 44", "wad"},
        {"00", "pic"},
        {"BE BA FE CA", "dba"},
        {"00 01 42 44", "dba"},
        {"00 01 44 54", "tda"},
        {"54 44 46 24", "tdf"},
        {"54 44 45 46", "tdef"},
        {"00 01 00 00", "palm"},
        {"00 00 01 00", "ico"},
        {"69 63 6E 73", "icns"},
        {"66 74 79 70 33 67", "3gp"},
        {"66 74 79 70 68 65 69 63", "heic"},
        {"1F 9D", "z"},
        {"1F A0", "z"},
        {"2D 68 6C 30 2D", "lzh"},
        {"42 41 43 4B 4D 49 4B 45", "bac"},
        {"49 44 58", "idx"},
        {"62 70 6C 69 73 74", "plist"},
        {"42 5A 68", "bz2"},
        {"47 49 46 38 37 61", "gif"},
        {"47 49 46 38 39 61", "gif"},
        {"49 49 2A 00", "tif"},
        {"4D 4D 00 2A", "tif"},
        {"49 49 2A 00 10 00 00 00", "cr2"},
        {"80 2A 5F D7", "cin"},
        {"52 4E 43 01", "rnc"},
        {"4E 55 52 55 49 4D 47", "nui"},
        {"53 44 50 58", "dpx"},
        {"76 2F 31 01", "exr"},
        {"42 50 47 FB", "bpg"},
        {"FF D8 FF DB", "jpg"},
        {"FF D8 FF E0 00 10 4A 46", "jpg"},
        {"FF D8 FF EE", "jpg"},
        {"FF D8 FF E1", "jpg"},
        {"00 00 00 0C 6A 50 20 20", "jp2"},
        {"FF 4F FF 51", "jp2"},
        {"71 6F 69 66", "qoi"},
        {"46 4F 52 4D", "ilbm"},
        {"4C 5A 49 50", "lz"},
        {"30 37 30 37 30 37", "cpio"},
        {"4D 5A", "exe"},
        {"53 4D 53 4E 46 32 30 30", "ssp"},
        {"5A 4D", "exe"},
        {"50 4B 03 04", "zip"},
        {"52 61 72 21 1A 07 00", "rar"},
        {"52 61 72 21 1A 07 01 00", "rar"},
        {"7F 45 4C 46", "elf"},
        {"89 50 4E 47 0D 0A 1A 0A", "png"},
        {"0E 03 13 01", "hdf4"},
        {"89 48 44 46 0D 0A 1A 0A", "hdf5"},
        {"C9", "com"},
        {"CA FE BA BE", "class"},
        {"EF BB BF", "txt"},
        {"FF FE", "txt"},
        {"FE FF", "txt"},
        {"FF FE 00 00", "txt"},
        {"00 00 FE FF", "txt"},
        {"2B 2F 76 38", "txt"},
        {"0E FE FF", "txt"},
        {"DD 73 66 73", "txt"},
        {"FE ED FA CE", "macho"},
        {"FE ED FA CF", "macho"},
        {"FE ED FE ED", "jks"},
        {"CE FA ED FE", "macho"},
        {"CF FA ED FE", "macho"},
        {"25 21 50 53", "ps"},
        {"25 50 44 46 2D", "pdf"},
        {"30 26 B2 75 8E 66 CF 11", "asf"},
        {"24 53 44 49 30 30 30 31", "sdi"},
        {"4F 67 67 53", "ogg"},
        {"38 42 50 53", "psd"},
        {"52 49 46 46", "avi"},
        {"FF FB", "mp3"},
        {"49 44 33", "mp3"},
        {"42 4D", "bmp"},
        {"43 44 30 30 31", "iso"},
        {"4C 00 00 00 01 14 02 00", "lnk"},
        {"62 6F 6F 6B 00 00 00 00", "alias"},
        {"75 73 74 61 72 00 30 30", "tar"},
        {"4D 53 43 46", "cab"},
        {"4B 44 4D", "vmdk"},
        {"43 72 32 34", "crx"},
        {"41 47 44 33", "fh8"},
        {"05 07 00 00 42 4F 42 4F", "cwk"},
        {"00 00 00 14 66 74 79 70", "mov"},
        {"4D 54 68 64", "mid"},
        {"4D 41 52 31 00", "mar"},
        {"4E 45 53 1A", "nes"},
        {"75 73 74 61 72", "tar"}
    }
    Private Function GetMagicByte(filePath As String) As String
        Try
            Using fs As New FileStream(filePath, FileMode.Open, FileAccess.Read)
                Dim buffer(0 To 15) As Byte
                fs.Read(buffer, 0, buffer.Length)

                Dim magicByteString = BitConverter.ToString(buffer).Replace("-", " ")
                For Each kvp In magicBytes
                    If magicByteString.StartsWith(kvp.Key) Then
                        Return kvp.Value
                    End If
                Next
            End Using
        Catch
            ' Ignore exceptions related to file access
        End Try
        Return Nothing
    End Function

    ' Checks if the file contains known magic bytes
    Private Function HasMagicBytes(filePath As String) As Boolean
        Dim magicByte = GetMagicByte(filePath)
        Return Not String.IsNullOrEmpty(magicByte)
    End Function

End Class
